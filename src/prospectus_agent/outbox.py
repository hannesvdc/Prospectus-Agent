...
def _recipient_line(contacts) -> str:
    ...
    EVERY unique address — published and all format guesses — so the one correct guess
    per person is guaranteed to be in there. Wrong guesses bounce; verify before
    sending if that matters."""
    seen, emails = set(), []
    for c in contacts:
        e = (c["email"] or "").strip()
